...
# Load C# parser assembly if available
$ParserDllPath = "$PSScriptRoot\bin\ADExplorerParser.dll"
if (Test-Path $ParserDllPath) {
    try {
        Add-Type -Path $ParserDllPath
        Write-Verbose "Loaded ADExplorerParser.dll successfully"
    }
    catch {
        Write-Warning "Failed to load ADExplorerParser.dll: $($_.Exception.Message)"
    }
}
